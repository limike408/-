...
def dp_report(price=None):
    ...
    ax1.set_xticks(list(hs)); ax1.set_xticklabels([f'{h:g}' for h in hs], fontsize=11)
    ax1.set_xlabel('DP 网格步长 h (kWh)', fontsize=12)
    ax1.set_ylabel('最小购电费 (元)', fontsize=12)
    ax1.set_title('(a) 附件1 典型日：DP 网格收敛', fontsize=13)
    ...
    ax2.set_xlabel('DP 相对 LP 偏差 (%)', fontsize=12)
    ax2.set_ylabel('天数', fontsize=12)
    ax2.set_title('(b) 附件2 全年逐日偏差分布 (h=15)', fontsize=13)
    ax2.legend(fontsize=11); ax2.grid(alpha=0.3, axis='y')
    ax2.tick_params(labelsize=11)
    fig.tight_layout()
