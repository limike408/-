...
def main():
    ...
    ax.set_xlabel('日期（2025 年逐日）', fontsize=12)
    ax.set_ylabel('净需求预测偏差 $\\varepsilon_d$', fontsize=12)
    ax.set_title(f'全年净需求预测偏差与「紧急波动日」识别（阈值 {THETA:.0%}）', fontsize=13)
    ax.legend(fontsize=11, loc='lower left')
    ax.tick_params(labelsize=11)
    ax.set_xticks(np.arange(0, n_total, 30))
    ax.set_xticklabels([frame['date'].iloc[i].strftime('%m-%d') for i in np.arange(0, n_total, 30)], rotation=45, fontsize=11)
    fig.tight_layout()
    ...
    for b, (r, c) in zip(bars, stat[['rate', 'sum']].itertuples(index=False)):
        ax.text(b.get_x() + b.get_width() / 2, b.get_height() + 0.3,
                f"{int(c)} 天 ({r:.0%})", ha='center', fontsize=12)
    ax.set_ylim(0, stat['sum'].max() * 1.15)
    ax.set_ylabel('紧急波动日数', fontsize=12)
    ax.set_title(f'各季节「紧急波动日」出现次数（低估净需求≥{THETA:.0%}）', fontsize=13)
