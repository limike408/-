...
\begin{abstract}
    ...
    负荷 15/光伏 7），在信息因果前提下得到全年计划购电费 $15133792.3$~元、紧急购电费
    $1074587.3$~元，合计 $16208379.6$~元。CVaR 敏感性分析表明：随风险厌恶权重 $\lambda$
    增大，计划购电费小幅上升、紧急购电显著削减，是高缺电风险日（如 12-21 处总费
    $62424\to60795$~元）降低总费用的有效“保险”手段。
    ...
    价下模型二计划购电费 $12846564.8$~元（较固定电价上浮 $4.8\%$）、模型三合计购电费
    $16046412.7$~元（上浮 $5.1\%$），并发现电价波动风险具有显著季节差异（冬季费用上浮达
    $12.1\%$）。
    ...
    风险—经济权衡，高缺电风险日 2025-12-21 总费用由 $62424$~元降至 $60795$~元；其三，提出
...
\textbf{经济性的理论下界}，可作为后续各问题在不确定性下的对照基准。需要指出的是，问题一假设电价、负荷、光伏全部已知，因此在现实调度中并不成立；它的
...
具体地，点预报取负荷、光伏各自前 $K_{\text{L}}/K_{\text{P}}$ 天逐时段（144）的均值，K 由全年
交叉验证按归一化 RMSE 从候选集 $\{7,15,30\}$ 选出（得 $K_{\text{L}}=15,\ K_{\text{P}}=7$）；
...
结算得紧急购电 $Q$，全年（2.1--12.31）计划购电费 $15133792.3$~元、紧急购电费
$1074587.3$~元（紧急购电 $346164$~kWh），合计 $16208379.6$~元。表~\ref{tab:q2} 给出四个
...
\begin{table}[H]
    ...
    \begin{tabular}{ccccc}
        ...
        \midrule[0.5pt]
        2025-03-20 & 42261.5 & 58.7 & 320.4 & 1200 \\
        2025-06-21 & 47902.4 & 0 & 0 & 1200 \\
        2025-09-23 & 40080.9 & 2562.6 & 7895.3 & 1200 \\
        2025-12-21 & 59561.4 & 1102.3 & 2862.7 & 1200 \\
        \bottomrule[0.8pt]
    \end{tabular}
\end{table}
...
上述建立与结果均以 $\lambda=0$（纯期望最小化）给出基准口径。为回答"当决策者愿意以少量
计划费用换取供电可靠性时，应如何权衡"，本文进一步引入风险权重 $\lambda>0$，使目标函数
式~\eqref{eq:obj2} 在计划费与紧急费之外纳入 CVaR 尾部项。针对四个指定日期，逐一求解
$\lambda\in\{0,0.5,1,2,5\}$，以其计划购电费 $P$ 对当日实际负荷/光伏结算，得到随 $\lambda$
演化的计划费—紧急费折中关系，其帕累托前沿见图~\ref{fig:pareto}，参数稳健性见第 6.2 节。
\begin{figure}[H]
    \centering
...
模型预测控制将动态优化与滚动更新结合，已被广泛用于微网调度\cite{cn:mpc2,cn:mpc}。设 0:00 发布
...
购电作为"兜底"，仅当预报误差显著增大（如寒潮、阴雨等天气突变日）才启用滚动调整}。原因在于，
紧急购电只在少数短时段缺电时触发、单次电量小、成本可控；而滚动调整受违约/超额惩罚牵制，
过度频繁反而抬升费用。故"大部分日子以计划购电 + 紧急兜底为主、预报误差大再调整"，既降低了
...
\begin{enumerate}
    ...
    \item \textbf{口径闭合}：全年各季节的汇总体费用之和与全年总费用一致，且
          ...
          避免因单位或日期错位产生口径偏差。
\end{enumerate}
上述校验通过，说明所得 35126.8 元（问题一）、16208379.6 元（问题二）、15263487.8 元
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule[0.5pt]
        计划+调整电网费 & 35126.8(单日) & 15133792.3 & 13008939.3 & 13591972.2 \\
        紧急购电费 & 0 & 1074587.3 & 2254548.5 & 2454440.5 \\
        全年合计 & -- & 16208379.6 & 15263487.8 & 16046412.7 \\
        \bottomrule[0.8pt]
    \end{tabular}
\end{table}
\begin{figure}[H]
